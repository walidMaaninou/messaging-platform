...
export default function MyAssets() {
  ...
  async function loadNFTs() {
    ...
    const data = await marketplaceContract.fetchMessageItems()
    console.log(data)
    const items = await Promise.all(data.map(async i => {
      ...
      console.log(meta)
      // let formattedFileLink = meta.data.file
      let item = {
  ...
  if (loadingState === 'loaded' && !nfts.length) return (<h1 className="py-10 px-20 text-3xl">No Public messages.</h1>)
  return (
            ...
            nfts.map((nft, i) => (
              <div key={i} className="border shadow rounded-xl overflow-hidden">
                ...
                <div className="p-4 bg-black">
                  ...
                  {nft.fileLink && <p className="text-xs font-bold text-white">Attachements: <a href={nft.fileLink.replace(".infura","")}>Click here</a></p>}
                </div>
              </div>
            ))
          }
